...
public class MyArrays2 {
	...
	public static float sumElemntsOfArray(float w[]) {
	...
	public static float sumElemntsOfArraysR(float w[], int pos) {
	...
	public static int foundElement(int z[], int x) {
		...
		for (int e = 0; e < z.length - 1 && noEsta; e++) { // En los arrays no se escribe length(), se hace sin los
															// parentesis
			if (z[e] == x) {
				noEsta = false;
				donde = e;
	...
	public static int foundElementR(double z[], double x, int pos) { // en recusividad se necesita una extra para la
																		// posición.
		if (pos >= z.length) {
			return -1;
		} else if (z[pos] == x) {
	...
	public static int[] foundElement2D(int M[][], int x) {
		// int donde[] = new int[2];
		int donde[] = { -1, -1 };
		boolean noEsta = true;
